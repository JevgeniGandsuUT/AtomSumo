...
| Version | 1.0 |
| Date | 2026-04-11 |
| Status | Ready for Verstapost 1 review |
| Primary architecture | M5 ATOM main controller + 2 x M5 AtomS3R-CAM + 2S 18650 power system + 4 x N20 6V drive motors |
...
- differential 4WD drive with four N20 6V DC gearmotors, wired as two left/right parallel motor pairs;
...
- [firmware/camera/sketch_jan17a/sketch_jan17a.ino](../../firmware/camera/sketch_jan17a/sketch_jan17a.ino)
...
- [hardware/pcb/Gerber_ATOM-Sumo_PCB_ATOM_SUMO_2026-04-08.zip](../../hardware/pcb/Gerber_ATOM-Sumo_PCB_ATOM_SUMO_2026-04-08.zip)
- [hardware/pcb/Gerber_ATOM-Sumo_PCB_ATOM_SUMO_2026-04-08-2.zip](../../hardware/pcb/Gerber_ATOM-Sumo_PCB_ATOM_SUMO_2026-04-08-2.zip)
- [media/video/MicrosoftTeams-video.mp4](../../media/video/MicrosoftTeams-video.mp4)
- [docs/assets/briefs/Versta1.png](../assets/briefs/Versta1.png)
...
- a recorded video artifact in `media/video/MicrosoftTeams-video.mp4` provides practical confirmation that streaming was demonstrated; the file is approximately `32 s` long.
- the original Verstapost 1 assignment/reference image is stored in `docs/assets/briefs/Versta1.png` so the repository keeps the submitted specification aligned with the course milestone brief.
...
### 3.4 Deviation from the Original Variant B Brief

The original Verstapost 1 sumo-robot brief proposes an older Android phone as the main robot brain. The reference build in this repository intentionally moves the embedded robot brain to an M5 ATOM / ESP32 controller and uses phones or laptops as browser clients only.

This deviation is acceptable for the selected prototype because it preserves the required system functions while reducing integration risk:

- the robot still provides remote operator control through a phone browser;
- the robot still has front vision through an `AtomS3R-CAM`;
- the robot still has downward sensing for floor color and future positioning;
- the robot still uses an ESP32-class controller for motor-driver interfacing;
- the control and camera firmware already exist and have been tested in the repository.

The Android-phone path remains a possible later UI/client option, but it is not required as the onboard compute platform for this reference build.
...
- The baseline OpenSCAD CAD contains an older powerbank-style bay, while the real Fusion 360 CAD package now includes battery-pack models. The final physical tray still needs print and service-access validation.
...
| FR-06 | The robot shall use differential drive with two independently controlled motor channels; each channel may drive a left/right pair of parallel motors. | Must | Bench test |
...
- 4 x `N20 6V` DC gearmotors (`N20` class footprint assumed by CAD);
- two-channel differential steering;
- 4 wheels total in the V1-V2 wheel baseline;
- 4 x `42 x 19 mm` wheels used as driven wheels;
...
- one left and one right driven wheel pair for propulsion and turning;
- skid/differential steering controlled by the two TB6612 channels.

The current wiring uses parallel motor pairs per side, so current draw and driver heating must be validated before high-load pushing tests.
...
The baseline OpenSCAD CAD still contains a `powerbank bay`. The newer real Fusion 360 CAD package includes battery-pack models, but the final physical `2S 18650` architecture shall still be validated as:
...
- left and right `N20 6V` motor pairs.
...
These pins are already used by the main controller firmware in [firmware/main/main.ino](../../firmware/main/main.ino).
...
- the final CAD battery packaging must be validated physically for `18650` service access;
- boundary/color telemetry exists, but closed-loop arena behavior is not yet completed.
...
| --- | --- | --- | --- | --- |
| M5 ATOM main controller | 1 | assumed available | existing stock / not counted | Working firmware already exists in repo |
| M5 AtomS3R-CAM | 2 | assumed available | existing stock / not counted | Working camera firmware already exists in repo |
...
1. Validate the final `2S 18650` mount physically for service access and impact safety.
2. Confirm gearbox ratio from calculation plus bench tests.
3. Validate whether the ordered `2S 3A` BMS and TB6612 channels can tolerate four-motor surge current.
4. Calibrate or explain the measured `VL53L0X` offset.
5. Convert downward color telemetry into closed-loop boundary/retreat behavior.
6. Calibrate click-to-drive using camera geometry or document the timed prototype limitation.

## 16. Revision Guidance for Verstapost 2

The minimum expected changes for the next revision of this specification are:

- replace remaining assumptions with measured values;
- add the exact battery holder geometry and updated chassis drawing;
- add measured motor speed, current, and push-force data;
- keep measured network latency statistics current;
- calibrate or explain measured ToF accuracy statistics;
- confirm BMS suitability under startup and stall current conditions.

## 17. Summary

This V1 specification defines a realistic and buildable sumo robot architecture that is now substantially implemented in the repository. The design is centered on an M5 ATOM controller, dual M5 AtomS3R-CAM Wi-Fi camera nodes, `2S 18650` battery power, browser-based teleoperation, ToF telemetry, Cam 2 color telemetry, and a path toward calibrated click-to-drive and arena-aware behavior.

The strongest current assets are the existing firmware, camera streaming, ToF/color telemetry, movement measurements, executed analysis notebooks, real Fusion CAD, and PCB files. The most important next steps are ToF calibration, camera-aware brownout retesting, enclosure/endurance validation, and closed-loop arena behavior.
